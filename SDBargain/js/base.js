...
(function ($) {
    function Popup(option) {
        ...
        var $modal, _this = this, html;
        var opts = $.extend(defaultOption, option);
        if ($('.popup-overlay').length == 0) {
            html += '<div class="popup-overlay"></div>';
        }
        html = '<div class="popup-modal ' + opts.type + '"><div class="modal-content"><div class="header">'
        ...
        if(opts.type == 'ttype' || opts.type == 'stype'){
            html += opts.content[0];
        }
        html += '</div><div class="close">取消</div></div></div>';
        $modal = $(html).appendTo(document.body);
